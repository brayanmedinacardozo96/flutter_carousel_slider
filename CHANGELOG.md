...
- Exposed `clipBehavior` in `AppCarouselOptions`
- Exposed `padEnds` in `AppCarouselOptions`
- Add `copyWith` method to `AppCarouselOptions`
...
Instead of passing all the options to the `CarouselSlider`, now you'll need to pass these option to `AppCarouselOptions`:

```dart
CarouselSlider(
  AppCarouselOptions(height: 400.0),
